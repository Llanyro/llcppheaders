...
static_assert(endsWith_01 == llcpp::FALSE, __LL_ERROR__);
static_assert(endsWith_02 == llcpp::TRUE , __LL_ERROR__);
static_assert(endsWith_03 == llcpp::TRUE , __LL_ERROR__);	// Here is different to start_with cause the '\0' at end
static_assert(endsWith_04 == llcpp::FALSE, __LL_ERROR__);	// Here is different to start_with cause the '\0' at end
...
constexpr auto shift_left_example() {
	...
	return algorithm::CompareCluster<len_t, len_t, llcpp::TRUE>().equals(block, block_cmp);
	//return false;
}
...
__LL_NODISCARD__ constexpr CompareResult compare(const T* v1, const len_t size1, const T* v2, const len_t size2) noexcept(true) {
	return (size1 == size2) ? compare<T>(v1, v2, size1) == 0 : llcpp::FALSE;
}

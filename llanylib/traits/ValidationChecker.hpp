...
class ValidationChecker {
	...
	private:
		...
		template<class T>
		__LL_NODISCARD__ static constexpr ::llcpp::ValidType getValidType() noexcept {
			...
			if constexpr (::llcpp::meta::traits::is_primitive_v<T>)
				return ::llcpp::ValidType::Primitive;
			else if constexpr (::std::is_array_v<T>)
				return ::llcpp::ValidType::Array;
			else if constexpr (::std::is_pointer_v<T>)
				return ::llcpp::ValidType::Pointer;
				...
				return ::llcpp::ValidType::Error;
			}
			else if constexpr (IS_VALID)
				return ::llcpp::ValidType::Valid;
			else if constexpr (IS_INVALID)
				return ::llcpp::ValidType::Invalid;
			else if constexpr (::llcpp::meta::concepts::signature::HasValidationType<T>)
				return ::llcpp::ValidType::ToCheck;
			else return ::llcpp::ValidType::Unknown;
		}
		...
		__LL_NODISCARD__ static constexpr ll_bool_t hasValidAttribute() noexcept {
			...
			//constexpr u8 MIN = static_cast<u8>(::llcpp::ValidType::Valid);
			constexpr u8 MAX = static_cast<u8>(::llcpp::ValidType::Error);
			return result <= MAX;
	...
	public:
		...
		template<class T>
		static constexpr ::llcpp::ValidType valid_type_v = _MyType::getValidType<T>();
		template<class T, ::llcpp::ValidType _TYPE>
